...
{   Copyright (c) 1998 by TMT Development Corporation                }
{   Copyright (c) 1992 by Borland International                      }
{                                                                    }
{   This program will work with both TMT Pascal and Borland Pascal   }
{                                                                    }
{********************************************************************}
...
uses
  {$ifdef __TMT__} Use32, {$endif} Crt, Graph;
...
{$IFDEF __TMT__}
  ModeNo: Word;
  ModesList: array [0..MaxVbeModes] of GraphModeType;
  Res, SummRes: DWord;
{$ENDIF}
...
begin
  ...
  Ch := ' ';
{$IFDEF __TMT__}
  if Resolution = Higher then begin
   GetVbeModesList(ModesList);
   SummRes:=0;
   ModeNo:=$13;
   for i:=0 to TotalVbeModes-1 do begin
    Res:=DWord(ModesList[i].XResolution)*DWord(ModesList[i].YResolution);
    if (SummRes<Res) and (ModesList[i].BitsPerPixel>4) then begin
      SummRes:=Res;
      ModeNo:=ModesList[i].VideoMode;
    end;
   end;
   SetGraphMode(ModeNo);
  end else SetSVGAMode(640,480,8,LfbOrBanked);
  if GraphResult<>0 then SetGraphMode($13);
  SetTextStyle(DefaultFont, HorizDir);
  MaxDelta:=16;
{$ELSE}
  GraphDriver := Detect;
...
begin
  if KeyPressed then begin
    Ch := ReadKey;
    if Upcase(Ch) = 'B' then
      SetRGBPalette(0,Random(63),Random(63),Random(63))
    else
