...
int G__blockscope::compile_core(int openBrace) {
  ...
  for(;;) {
    ...
    if(c==';'|| c=='}') {
#ifdef G__OLDIMPLEMENTATION2134
      if(c==';') m_bc_inst.CL();
#endif
      if(!openBrace) break;
      else c=0;
...
int G__blockscope::compile_column(string& token,int c) {
  ...
  else if(token=="public")    token.clear();
  else if(token=="protected") token.clear();
  else if(token=="private")   token.clear();
  else {
    //
    c = m_preader->fgetc();
    if(c==':') {
...
int G__blockscope::compile_return(string& token,int c) {
  token.clear();
#ifndef G__OLDIMPLEMENTATION2183
  int c2;
...
int G__blockscope::compile_declaration(G__TypeReader& type,string& token,int c) {
  ...
  if(
#ifndef G__OLDIMPLEMENTATION2127
     type.Isstatic()
#else
     (type.Isfundamental() && 
        (type.Isstatic() || type.Isconst())) || type.Isstatic() 
#endif
     ) {
...
#else
      int hash = ifunc->hash[ifn];
      m_bc_inst.LD_IFUNC(ifunc,ifn,hash,libp->paran,G__EXACT
			,G__CALLCONSTRUCTOR);
#endif
...
#else
      int hash = ifunc->hash[ifn];
      m_bc_inst.LD_IFUNC(ifunc,ifn,hash,libp->paran,G__EXACT ,memfuncflag);
#endif
    }
    else {
      // This is for interpreted class
...
struct G__var_array* G__blockscope::allocatevariable(G__TypeReader& type
  ...
  var->p_tagtable[ig15] = type.Tagnum();
  if(type.Isreference()) {
...
#else
      int hash = ifunc->hash[ifn];
      m_bc_inst.LD_IFUNC(ifunc,ifn,hash,para.paran,G__EXACT
			  ,G__CALLCONSTRUCTOR);
#endif
    }
    else {
...
#ifndef G__OLDIMPLEMENTATION2187
  m_bc_inst.PUSHCPY();
...
#else
      int hash = ifunc->hash[ifn];
      m_bc_inst.LD_IFUNC(ifunc,ifn,hash,0,G__EXACT ,G__TRYDESTRUCTOR);
#endif
    }
    else {
...
extern "C" void G__bc_delete_operator(const char *expression,int isarray) {
  ...
  return(G__currentscope->compile_deleteopr(expr,isarray));
}
